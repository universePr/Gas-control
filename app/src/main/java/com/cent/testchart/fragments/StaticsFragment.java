...
public class StaticsFragment extends Fragment {
    ...
    private void init_() {
        ...
        cartesian.title(getString(R.string.title_static_chart));
        ...
        if(list_carbon_monoxide.size() == 0 ){
         container.findViewById(R.id.tv_found_data).setVisibility(View.VISIBLE);
        }else {
            container.findViewById(R.id.tv_found_data).setVisibility(View.GONE);
            for (int i = 0; i < list_carbon_monoxide.size(); i++) {
    ...
    private void init_arrays() {
        list_carbon_monoxide = commit2DB.getLatest7Days(tag_co);
        list_lpg = commit2DB.getLatest7Days(tag_lpg);
        list_smoke = commit2DB.getLatest7Days(tag_smoke);
